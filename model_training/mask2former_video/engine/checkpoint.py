...
class Checkpointer:
    ...
    def load(
        ...
        # If any of the self.model.state_dict() keys contains "teacher" or "student",
        # we assume that the model is a student-teacher model and we modify the
        # checkpoint keys accordingly.
        if (any(
            key.startswith("student.") or key.startswith("teacher.")
            for key in self.model.state_dict().keys()
        ) and not any(key.startswith("student.") or key.startswith("teacher.")
                for key in checkpoint.get("model", {}).keys())):

            print("--- USING NEW MODEL LOADING FOR STUDENT AND TEACHER ---")
            checkpoint = self.modify_checkpoint_keys_for_student_teacher(checkpoint)
        ...
        if (any(
            ...
            for key in checkpoint.get("model", {}).keys()
                ...
                for key in self.model.state_dict().keys())):
            print("--- CONVERTING DISTILLATION CHECKPOINT TO NORMAL VM2F LOADING ---")
            checkpoint = self.convert_distillation_checkpoint_to_normal_vm2f(checkpoint)
    ...
    def modify_checkpoint_keys_for_student_teacher(self, checkpoint):
        """
        Modifies the checkpoint keys from a standard Mask2Former model to a
        student-teacher setup.

        The original checkpoint has keys prefixed with "backbone" and "sem_seg_head".
        The new model has a "student" and "teacher" attribute, each being
        a torch.nn.Sequential container of the backbone and head.

        Args:
            checkpoint (dict): The original checkpoint state dictionary.

        Returns:
            dict: A new state dictionary with keys modified to match the
                  student-teacher model.
        """
        # Extract the state_dict, assuming it might be nested under a 'model' key.
        # If not, we just use the checkpoint dictionary itself.
        ...
        # Iterate through the old state dictionary and rename keys
        for k, v in old_state_dict.items():
            # Check if the key belongs to the backbone
            if k.startswith("backbone."):
                # New key for the student model's backbone (index 0 in Sequential)
                new_k_student = k.replace("backbone.", "student.0.")
                new_state_dict[new_k_student] = v

                # New key for the teacher model's backbone (also index 0)
                new_k_teacher = k.replace("backbone.", "teacher.0.")
                new_state_dict[new_k_teacher] = v

            # Check if the key belongs to the semantic segmentation head
            elif k.startswith("sem_seg_head."):
                # New key for the student model's head (index 1 in Sequential)
                new_k_student = k.replace("sem_seg_head.", "student.1.")
                new_state_dict[new_k_student] = v

                # New key for the teacher model's head (also index 1)
                new_k_teacher = k.replace("sem_seg_head.", "teacher.1.")
                new_state_dict[new_k_teacher] = v
    ...
    def convert_distillation_checkpoint_to_normal_vm2f(self, checkpoint):
        """
        Modifies the checkpoint keys from a student-teacher setup Mask2Former model to a
        standard Mask2Former model
        .

        The original checkpoint has keys prefixed with "backbone" and "sem_seg_head".
        The distillation model has a "student" and "teacher" attribute, each being
        a torch.nn.Sequential container of the backbone and head.
        ...
        # Iterate through the old state dictionary and rename keys from the teacher model
        for k, v in old_state_dict.items():
            if k.startswith("teacher."):
                # Keys for the backbone, e.g., "teacher.0.res2..." -> "backbone.res2..."
                if k.startswith("teacher.0."):
                    new_k = k.replace("teacher.0.", "backbone.", 1)
                    new_state_dict[new_k] = v
                # Keys for the segmentation head, e.g., "teacher.1.pixel_decoder..." -> "sem_seg_head.pixel_decoder..."
                elif k.startswith("teacher.1."):
                    new_k = k.replace("teacher.1.", "sem_seg_head.", 1)
                    new_state_dict[new_k] = v
                elif k.startswith("criterion."):
                    new_k = k.replace("criterion.", "criterion.", 1)
                    new_state_dict[new_k] = v
