...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def _derive_coco_results(self, coco_eval, class_names=None):
        ...
        Args:
            ...
            class_names (None or list[str]): if provided, will be used to predict
                per-category AP.
        ...
        # Print the standard metrics, like YTVOSeval.summarize() does
        coco_eval.summarize()

        # Create a comma-separated string of the stats, similar to print_and_summary
        copypaste_results = ",".join([f"{s * 100:.2f}" for s in coco_eval.stats])
        self._logger.info(f"Copy-paste friendly results: {copypaste_results}")

        # The standard metrics
        metrics = ["AP", "AP50", "AP75", "APs", "APm", "APl", "AR1", "AR10", "AR100"]
        results = {
            metric: float(coco_eval.stats[idx] * 100 if coco_eval.stats[idx] >= 0 else "nan")
            for idx, metric in enumerate(metrics) if idx < len(coco_eval.stats)
        }

        if not np.isfinite(sum(results.values())):
            self._logger.info("Some metrics could not be computed and are shown as NaN.")
    ...
    def _derive_coco_results_orig(self, coco_eval, class_names=None):
        """
        Derive the desired score numbers from summarized COCOeval.
        Args:
            coco_eval (None or COCOEval): None represents no predictions from model.
