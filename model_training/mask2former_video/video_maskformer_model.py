...
from .utils.memory import retry_if_cuda_oom, model_retry_if_cuda_oom
...
class VideoMaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        num_predictions = cfg.MODEL.MASK_FORMER.TEST.NUM_PREDICTIONS

        accum_iter = cfg.SOLVER.ACCUM_ITER

        if cfg.MODEL.MASK_FORMER.SPARSE_CLASS_WEIGHT > 0.:
            matcher = VideoHungarianMatcher(
                cost_class=cfg.MODEL.MASK_FORMER.SPARSE_CLASS_WEIGHT
                if not cfg.MODEL.MASK_FORMER.NO_CLASS_MATCH else 0.,
        ...
        weight_dict = {"loss_ce": class_weight, "loss_mask": mask_weight, "loss_dice": dice_weight,
                       "loss_ce_sparse": cfg.MODEL.MASK_FORMER.SPARSE_CLASS_WEIGHT, "loss_entropy": cfg.MODEL.MASK_FORMER.ENTROPY_WEIGHT}
        ...
        #losses = ["labels", "masks", "labels_sparse", "entropy"]
        losses = ["labels", "masks"]
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            features = model_retry_if_cuda_oom(self.backbone)(images.tensor)
            outputs = model_retry_if_cuda_oom(self.sem_seg_head)(features)
    ...
    def prepare_targets(self, targets, images):
        ...
        for i, targets_per_video in enumerate(targets):
            ...
            #print("IN PREPARE TARGETS")
            #print("targets_per_frame.gt_classes", targets_per_frame.gt_classes)
            gt_classes_per_video = targets_per_frame.gt_classes[valid_idx]     # N,
            #print("gt_classes_per_video", gt_classes_per_video)
    ...
    def inference_video(self, pred_cls, pred_masks, img_size, output_height, output_width):
        if len(pred_cls) > 0:
            scores = F.softmax(pred_cls, dim=-1)[:, :-1] # All scores except "no-object" class
            labels = torch.arange(self.sem_seg_head.num_classes, device=self.device).unsqueeze(0).repeat(self.num_queries, 1).flatten(0, 1)

            scores_per_image, topk_indices = scores.flatten(0, 1).topk(self.num_predictions, sorted=True)
