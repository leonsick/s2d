...
from mask2former_video import kd_video_maskformer_model
...
# additional settings
from mask2former_video.engine import DefaultTrainer as M2FTrainer
import wandb

from detectron2.utils.events import EventWriter, get_event_storage

from cutler.modeling import roi_heads
from cutler.config import add_cutler_config
from cutler.engine import default_argument_parser, default_setup
from cutler.engine import DefaultTrainer as CutLERTrainer


class Trainer(M2FTrainer):
    """
    ...
    def build_train_loader(cls, cfg):
        if len(cfg.DATASETS.TRAIN) == 1:
            ...
            for dataset_name in dataset_names:
                dataset_dicts.extend(get_detection_dataset_dicts(
                    dataset_name,
                    filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
                    proposal_files=cfg.DATASETS.PROPOSAL_FILES_TRAIN if cfg.MODEL.LOAD_PROPOSALS else None,
                ))
...
class WandBTrainer(Trainer):
    ...
    def build_writers(self):
        ...
        """
        from mask2former_video.engine.defaults import default_writers
...
def setup(args):
    ...
    # Process opts to cast WEIGHT parameters to float
    opts = list(args.opts)
    for i in range(0, len(opts), 2):
        key = opts[i]
        if "WEIGHT" in key:
            try:
                # Cast the value to float and update it in the list
                opts[i + 1] = float(opts[i + 1])
            except (ValueError, TypeError):
                # In case the value is not a valid number, skip it
                pass

    cfg.merge_from_list(opts)
    # NOTE: need to add following lines to detectron2/detectron2/engine/defaults.py
...
def main(args):
    ...
    if not args.eval_only and args.wandb:
    ...
    elif args.eval_only and args.wandb:
        if comm.is_main_process():  # only on main process
            wandb.init(
                # set the wandb project where this run will be logged
                project="motionseg_m2f_evalonly",
                sync_tensorboard=True,
                config=cfg,
                name="VideoM2F_Training"
            )
